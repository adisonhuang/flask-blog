...
$(document).ready(function() {
    $('.add-category-btn').click(function() {
        $('#addCategoryFormModel').modal();
    });
});

//JS For confirm to delete an category
function delCategoryCfm(url) {
    $('#delCategoryCfmClick').click(function(){
        window.location.href = url;
    });
    $('#delCategoryCfmModel').modal();
}

//JS For edit category to get its info
function get_category_info(url, id) {
    $.getJSON(url, function(data) {
        ...
        $('#editCategoryFormModel').modal();
    });
}

//JS For add category
$(document).ready(function() {
    $('.add-category-nav-btn').click(function() {
        $('#addCategoryNavFormModel').modal();
    });
});

//JS For edit categoryNav to get its info
function get_categoryNav_info(url) {
    $.getJSON(url, function(data) {
        $('#editNavName').val(data.name);
        $('#nav_id').val(data.nav_id);
        $('#NavModalTitle').text('修改分类导航：' + data.name);
        $('#editCategoryNavFormModel').modal();
    });
}

//JS For confirm to delete an category nav
function delCategoryNavCfm(url) {
    $('#delCategoryNavCfmClick').click(function(){
        window.location.href = url;
    });
    $('#delCategoryNavCfmModel').modal();
}
